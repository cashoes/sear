...
system.time({
  genesets_links <- t(combn(as.numeric(names(foo)), 2)) %>%
    ...
    # dplyr::rowwise(.) %>%
    # dplyr::mutate(jaccard  = jaccard(unlist(foo[source + 1]), unlist(foo[target + 1]))) %>%
    dplyr::mutate(jaccard  = unlist(map2(foo[as.character(source)], foo[as.character(target)], jaccard)))
...
system.time(nodes_small <- process_nodes(genesets %>% dplyr::filter(collection == 'TISSUES')))
system.time(links_small <- process_links(nodes_small, genesets_links) %>% filter(jaccard > 0.1))

forceNetwork(Links = links_small,
             Nodes = nodes_small,
             NodeID = 'geneset', Nodesize = 'size', Group = 'subcollection',
             Source = 'source', Target = 'target', Value = 'jaccard',
             linkDistance = JS("function(d) { return (1 / d.value) * 40; }"),
             fontSize = 24, fontFamily = 'sans-serif', opacity = 0.75,
             zoom = T, legend = T, bounded = T, opacityNoHover = 0.1,
             charge = -250) %>%
